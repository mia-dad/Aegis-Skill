...
public class DbSelectTool extends AbstractDbTool {
    ...
    public ToolSchema getOutputSchema() {
        Map<String, ToolSchema.ParameterSpec> parameters = new HashMap<String, ToolSchema.ParameterSpec>();
        parameters.put("rows", ToolSchema.ParameterSpec.required(
                "array", "查询结果行列表，每行为一个字段值映射"));
        parameters.put("rowCount", ToolSchema.ParameterSpec.required(
                "integer", "返回的记录数"));
        return new ToolSchema(parameters);
    }
    ...
    public void execute(Map<String, Object> input, ToolOutputContext output) {
        ...
        try {
            ...
            // 写入输出上下文（直接传递 List 对象，不序列化为 JSON 字符串）
